...
class CustomerServiceAPIClient:
    ...
    def _parse_final_event(self, data: Dict[str, Any]) -> Optional[str]:
        ...
        if data.get("type") == "json" and "content" in data:
            ...
            return content.get("prefixText", str(content))
    ...
    def _process_event(
        ...
        if event_type == EventType.FINAL.value:
            response.message = self._parse_final_event(data)
...
def call_customer_service_api(
    ...
    with CustomerServiceAPIClient() as client:
        ...
        return TaskResult(
            ...
            message=response.metadata.get("raw_content", "")
        )
...
def main():
    ...
    print("-" * 50)
    result = call_customer_service_api(message="What did I ask you now?", session_id="d205fd2c-7671-466c-b8a5-774eac5e710e")
    print(result)
