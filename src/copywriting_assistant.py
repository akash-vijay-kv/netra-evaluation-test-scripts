...
async def ask_copywriting_agent(query: str, model: Optional[str] = None) -> str:
    with Netra.start_span("generation_pipeline"):
        ...
        # Refactor Copywrite
        content = refactoring_service(checked_content, model=model)
        return content
...
async def get_copywriting_agent_response(
    ...
    return await ask_copywriting_agent(query=query, model=model)


if __name__ == "__main__":
    asyncio.run(get_copywriting_agent_response(query="Write an engaging and fun blurb about our brand new chopsticks."))